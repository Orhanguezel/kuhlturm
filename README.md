...
Soğutma kulesi (Kühlturm) çözümleri için **B2B web platformu** — Almanca + İngilizce pazar sitesi. ensotek_de mimarisinin neredeyse aynısı; bağımsız veritabanıyla Ensotek alt markasıdır; sosyal yayın tek Ensotek hesabından yapılır.
...
Veritabanı: MySQL — **`kuhlturm_live`** (ensotek_de'nin `ensotek` DB'sinden tamamen ayrı).
...
- `backend/.env.example` — lokal DB=kuhlturm, canlı DB=kuhlturm_live, PORT=8089, locale DE+EN
...
> Şema değişikliği: `ALTER TABLE` kullanılmaz. İlgili `src/db/seed/sql/0XX_*_schema.sql` güncellenir, `db:seed:*:fresh` yalnız izole geliştirme veritabanında kullanılır. Canlı veri sıfırlanmaz.

## Köken (Origin) — ensotek_de'den klonlandı

backend ve admin_panel iskeletleri `ensotek_de`'den kopyalandı; kritik configler (paket isimleri, portlar, ecosystem cwd, env, DB adı) kuhlturm'a göre uyarlandı. Marka ismi (Ensotek → Kühlturm), TR locale temizliği, seed branding normalizasyonu ve frontend PM2 konfigürasyonu Codex tarafında ele alındı. Ürün ve servis modülleri, mevcut native sözleşmeyi korumak için yereldir; genel shared ürün/servis şeması bu veritabanıyla uyumlu değildir. Tarihsel bağlam: [`docs/kuhlturm-codex-brief.md`](docs/kuhlturm-codex-brief.md).
...
Güncel dağıtım, yedek, veri sahipliği ve kabul: [10 Eylül işletim kaydı](docs/runtime-isolation-2026-09-10.md).
