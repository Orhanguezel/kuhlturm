...
export const createPreferencesStore = (init?: PreferencesInit) =>
  createStore<PreferencesState>()((set) => ({
    ...
    themePreset:
      (init?.themePreset ?? (readCookieValue("theme_preset") as ThemePreset)) || PREFERENCE_DEFAULTS.theme_preset,
    font: (init?.font ?? (readCookieValue("font") as FontKey)) || PREFERENCE_DEFAULTS.font,
    contentLayout:
      (init?.contentLayout ?? (readCookieValue("content_layout") as ContentLayout)) ||
      PREFERENCE_DEFAULTS.content_layout,
    navbarStyle:
      (init?.navbarStyle ?? (readCookieValue("navbar_style") as NavbarStyle)) || PREFERENCE_DEFAULTS.navbar_style,
    sidebarVariant:
      (init?.sidebarVariant ?? (readCookieValue("sidebar_variant") as SidebarVariant)) ||
      PREFERENCE_DEFAULTS.sidebar_variant,
    sidebarCollapsible:
      (init?.sidebarCollapsible ?? (readCookieValue("sidebar_collapsible") as SidebarCollapsible)) ||
