...
export default function AdminReferencesClient() {
  ...
  const listQ = useListReferencesAdminQuery(
    queryParams as any,
    {
  ...
  async function onTogglePublished(item: ReferenceDto, next: boolean) {
    ...
    try {
      ...
      toast.error(err?.data?.error?.message || err?.message || t("admin.references.list.statusUpdateError"));
    }
  }
  ...
  async function onDelete(item: ReferenceDto) {
    ...
    const ok = window.confirm(t("admin.references.list.deleteConfirm", { name: String(item?.title ?? "Referans") }));
    if (!ok) return;
    ...
    try {
      ...
      toast.error(err?.data?.error?.message || err?.message || t("admin.references.list.deleteError"));
    }
  }
  ...
  const resolveImg = (item: ReferenceDto) => (item as any).featured_image_url_resolved || item.featured_image || "";

  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="font-semibold text-lg">{t("admin.references.header.title")}</h1>
        <p className="text-muted-foreground text-sm">{t("admin.references.header.description")}</p>
      </div>
        ...
        <CardHeader className="gap-2">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div className="space-y-1">
              ...
              <CardDescription>
                {t("admin.references.header.totalLabel")} <span className="font-medium">{total}</span> •{" "}
                {t("admin.references.header.activeLocale")} <Badge variant="secondary">{effectiveLocale || "—"}</Badge>
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="published">{t("admin.references.header.statusActive")}</SelectItem>
                  <SelectItem value="draft">{t("admin.references.header.statusInactive")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground text-xs">{t("admin.references.header.localeUrlSync")}</p>
            </div>
            ...
            <div className="flex items-center gap-2 pt-6 md:pt-7">
              <Switch
                ...
                disabled={busy}
              />
              <span className="text-muted-foreground text-sm">{t("admin.references.header.featuredOnly")}</span>
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">{t("admin.references.list.actionsColumn")}</TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    <TableRow key={id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          />
                          {isPublished ? (
                            <Badge variant="secondary">{t("admin.references.list.activeStatus")}</Badge>
                          ) : (
                            <Badge variant="destructive">{t("admin.references.list.inactiveStatus")}</Badge>
                          )}
                        </div>
                      </TableCell>
          ...
          <div className="rounded-md border md:hidden">
            <div className="divide-y">
              {items.map((item) => {
                ...
                const slug = String(item?.slug ?? "").trim() || "—";
                const isPublished = isTruthyBoolLike((item as any)?.is_published);
                const _isFeatured = isTruthyBoolLike((item as any)?.is_featured);
