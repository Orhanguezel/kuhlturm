...
export function SmtpTestSection({ busy }: SmtpTestSectionProps) {
  ...
  return (
    <div className="space-y-3">
      <Label className="font-medium text-sm">{t("admin.siteSettings.smtp.testEmail")}</Label>
      <div className="flex flex-col gap-2 sm:flex-row">
        <Input
          ...
          type="email"
        />
        <Button type="button" variant="outline" onClick={handleTest} disabled={busy || testing || !testEmail.trim()}>
          {testing ? t("admin.siteSettings.smtp.sendingTest") : t("admin.siteSettings.smtp.sendTest")}
        </Button>
      </div>
      {result ? (
        <p className={`text-xs ${result.ok ? "text-green-600" : "text-destructive"}`}>{result.message}</p>
      ) : null}
    </div>
  );
}
