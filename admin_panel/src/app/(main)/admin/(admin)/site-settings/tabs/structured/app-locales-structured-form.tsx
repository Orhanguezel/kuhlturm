...
export const AppLocalesStructuredForm: React.FC<AppLocalesStructuredFormProps> = ({ value, onChange, disabled }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={`${item.code}-${i}`} className="flex items-center gap-3 rounded-md border px-3 py-2">
            <span className="w-8 text-center font-medium font-mono text-sm uppercase">{item.code}</span>
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                disabled={disabled}
              />
              <Label className="w-16 text-[10px]">{t("admin.siteSettings.structured.appLocales.defaultLabel")}</Label>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                disabled={disabled}
              />
              <Label className="w-8 text-[10px]">{t("admin.siteSettings.structured.appLocales.activeLabel")}</Label>
            </div>
            ...
            <Button
              ...
              title={
                item.is_default
                  ? t("admin.siteSettings.structured.appLocales.defaultCannotDelete")
                  : t("admin.siteSettings.structured.appLocales.delete")
              }
            >
      ...
      {availableToAdd.length > 0 && (
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground text-xs">{t("admin.siteSettings.structured.appLocales.addNew")}</span>
        </div>
      )}
    </div>
  );
