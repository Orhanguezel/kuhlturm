...
export default function AdminAuditClient() {
  ...
  }, [event, email, user_id, ip, from, to, limit, offset, ALL]);
  ...
  const reqQ = useListAuditRequestLogsAdminQuery(tab === "requests" ? (reqParams as any) : (undefined as any), {
    skip: tab !== "requests",
    refetchOnFocus: true,
  } as any) as any;

  const authQ = useListAuditAuthEventsAdminQuery(tab === "auth" ? (authParams as any) : (undefined as any), {
    skip: tab !== "auth",
    refetchOnFocus: true,
  } as any) as any;

  const metricsQ = useGetAuditMetricsDailyAdminQuery(tab === "metrics" ? (metricsParams as any) : (undefined as any), {
    skip: tab !== "metrics",
    refetchOnFocus: true,
  } as any) as any;
  ...
  const geoQ = useGetAuditGeoStatsAdminQuery(tab === "map" ? (geoParams as any) : (undefined as any), {
    skip: tab !== "map",
    refetchOnFocus: true,
  } as any) as any;
                          ...
                          <TableCell className="text-sm">
                            ...
                            {r.referer && (
                              <div className="text-muted-foreground text-xs">
                                ref: {truncateNullable(r.referer, 40)}
                              </div>
                            )}
                          </TableCell>
                          <TableCell>
