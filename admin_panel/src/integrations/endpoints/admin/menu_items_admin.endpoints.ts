...
// Kühlturm – Admin Menu Items RTK endpoints
// =============================================================
...
export const menuItemsAdminApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    listMenuItemsAdmin: build.query<MenuItemListResponse<AdminMenuItemDto>, AdminMenuItemListQueryParams | undefined>({
    ...
    updateMenuItemAdmin: build.mutation<AdminMenuItemDto, { id: string; data: AdminMenuItemUpdatePayload }>({
      query: ({ id, data }) => ({
        url: `/admin/menu_items/${encodeURIComponent(id)}`,
        method: "PATCH",
        body: data,
