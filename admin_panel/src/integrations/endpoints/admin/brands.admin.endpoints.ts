...
import { toAdminBrandQuery, unwrap } from "@/integrations/shared";
...
export const brandsAdminApi = baseApi.injectEndpoints({
  endpoints: (b) => ({
    listBrandsAdmin: b.query<BrandListResult, BrandListParams | undefined>({
      query: (p) => ({ url: BRANDS, params: toAdminBrandQuery(p) }),
      transformResponse: (raw: any, meta) => {
        const items = Array.isArray(raw) ? (raw as BrandLogoMerged[]) : unwrap<BrandLogoMerged[]>(raw);
      ...
      providesTags: (res) =>
        res?.items?.length
          ? [{ type: "Brand" as const, id: "LIST" }, ...res.items.map((x) => ({ type: "Brand" as const, id: x.id }))]
    ...
    removeBrandAdmin: b.mutation<{ ok: true } | undefined, { id: string }>({
