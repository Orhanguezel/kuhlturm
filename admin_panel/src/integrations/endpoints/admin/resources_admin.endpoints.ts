...
export const resourcesAdminApi = baseApi.injectEndpoints({
  endpoints: (b) => ({
    ...
    listResourcesAdmin: b.query<ResourceAdminListItemDto[], ResourcesAdminListQueryParams | undefined>({
      query: (params) => ({
        url: BASE,
        method: "GET",
    ...
    updateResourceAdmin: b.mutation<ResourceRowDto, { id: string; patch: ResourceUpdatePayload }>({
      query: ({ id, patch }) => ({
        url: `${BASE}/${encodeURIComponent(id)}`,
        method: "PATCH",
        body: patch,
    ...
    deleteResourceAdmin: b.mutation<{ ok: boolean } | undefined, string>({
      query: (id) => ({
        url: `${BASE}/${encodeURIComponent(id)}`,
        method: "DELETE",
      }),
