...
export const referencesAdminApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    /* -------------------- LIST (admin) -------------------- */
    listReferencesAdmin: build.query<ReferenceListResponse, ReferenceListQueryParams | undefined>({
    ...
    /* -------------------- GET BY ID (admin) ---------------- */
    getReferenceAdmin: build.query<ReferenceDto, { id: string | number; locale?: string } | string | number>({
      query: (arg) => {
      ...
      providesTags: (_res, _err, arg) => {
        const id = typeof arg === "string" || typeof arg === "number" ? String(arg) : String(arg.id);
    ...
    /* -------------------- UPDATE (admin) ------------------- */
    updateReferenceAdmin: build.mutation<ReferenceDto, { id: string; patch: ReferenceUpsertPayload }>({
      query: ({ id, patch }) => ({
    ...
    listReferenceImagesAdmin: build.query<ReferenceImageDto[], string>({
      ...
      providesTags: (_res, _err, referenceId) => [{ type: "AdminReferenceImages", id: referenceId }],
    }),
