...
export const telegramInboundAdminApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    listTelegramInbound: build.query<TelegramInboundListResult, TelegramInboundListParams | undefined>({
      query: (arg) => {
...
export const { useListTelegramInboundQuery, useGetTelegramAutoReplyQuery, useUpdateTelegramAutoReplyMutation } =
