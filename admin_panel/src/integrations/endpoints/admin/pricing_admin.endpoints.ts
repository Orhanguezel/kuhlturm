...
export const pricingAdminApi = baseApi.injectEndpoints({
  endpoints: (b) => ({
    ...
    listPricingPlansAdmin: b.query<PricingPlanAdmin[], PricingPlanListParams | undefined>({
      query: (p) => (p ? { url: BASE, params: toAdminPricingQuery(p) as Record<string, any> } : { url: BASE }),
    ...
    updatePricingPlanAdmin: b.mutation<PricingPlanAdmin, { id: string; patch: PatchPricingPlanInput }>({
      query: ({ id, patch }) => ({
        url: `${BASE}/${id}`,
        method: "PATCH",
        body: patch,
