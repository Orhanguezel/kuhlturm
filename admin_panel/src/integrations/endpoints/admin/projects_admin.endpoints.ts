...
export const projectsAdminApi = baseApi.injectEndpoints({
  endpoints: (b) => ({
    // GET /admin/projects
    listProjectsAdmin: b.query<Project[], ProjectListParams | undefined>({
      query: (p) => (p ? { url: `${BASE}`, params: toAdminProjectQuery(p) as Record<string, any> } : { url: BASE }),
    ...
    listProjectImagesAdmin: b.query<ProjectImage[], string>({
      ...
      providesTags: (_res, _e, projectId) => [{ type: "ProjectImages" as const, id: `p:${projectId}` }],
    }),

    // POST /admin/projects/:id/images
    createProjectImageAdmin: b.mutation<ProjectImage[], { projectId: string; body: UpsertProjectImageInput }>({
      query: ({ projectId, body }) => ({
        url: `${BASE}/${projectId}/images`,
        method: "POST",
        body,
      }),
      invalidatesTags: (_res, _e, { projectId }) => [{ type: "ProjectImages" as const, id: `p:${projectId}` }],
    }),
